view: daily_cac_last_touch_channel {
  derived_table: {
    sql: select coalesce(a.date, b.date) as date,
               ...
               coalesce(a.last_touch_channel, b.renamed_channel) as channel,
               isnull(new_users, 0) as new_users,
               isnull(daily_spend, 0) as daily_spend
          from (
          select date(client_created_pst) date, region, last_touch_channel, count(distinct client_id) as new_users
          ...
          ) b on a.date = b.date and a.region = b.region and a.last_touch_channel = b.renamed_channel
          order by 1
 ;;
  }
