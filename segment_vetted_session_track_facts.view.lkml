...
view: segment_vetted_session_track_facts {
  derived_table: {
    sql_trigger_value: select count(*) from ${segment_vetted_track_facts.SQL_TABLE_NAME} ;;
    sortkeys: ["session_id"]
    ...
    sql: SELECT s.session_id
      ...
      FROM ${segment_vetted_session_tracks.SQL_TABLE_NAME} AS s
      LEFT JOIN ${segment_vetted_track_facts.SQL_TABLE_NAME} as map on map.session_id = s.session_id
      GROUP BY 1
       ;;
  }
